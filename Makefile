...
crc-restore-pinning: ## restore web console developer pinning. Need to login / pin something before
	oc -n openshift-console-user-settings \
		...
		-p '{"data":{"console.pinnedResources":"{\"admin\":[],\"dev\":[\"core~v1~ConfigMap\",\"apps~v1~Deployment\",\"networking~v1~Ingress\",\"mailhog.operators.patrick.mx~v1alpha1~MailhogInstance\",\"core~v1~Service\",\"core~v1~Pod\",\"route.openshift.io~v1~Route\"]}"}}'
...
.PHONY: bundle
bundle: manifests kustomize create-kustomize-release-patch ## Generate bundle manifests and metadata, then validate generated files.
...
.PHONY: scorecard-basic-spec-test
